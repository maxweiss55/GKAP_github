...
calc11 <- clean11 %>%
  mutate(Response_Legal = case_when(Response_Legal == 1 ~ '0',
                                    Response_Legal == 2 ~ '1',
                                    Response_Legal == 3 ~ '2',
                                    Response_Legal == 4 ~ '3',
                                    Response_Legal == -1 ~ NA_character_)) %>%
  mutate(Response_Legal = parse_integer(Response_Legal)) %>%
  mutate(Response_Medical = case_when(Response_Medical == 1 ~ '0',
                                    Response_Medical == 2 ~ '1',
                                    Response_Medical == 3 ~ '2',
                                    Response_Medical == 4 ~ '3',
...
calc11 %>%
  filter(!is.na(Response_Medical), !is.na(Response_Legal)) %>%
  summarise(Legal_Favorability = mean(Response_Legal), Medical_Favorability = mean(Response_Medical)) %>%
  gather(Legal_Favorability, Medical_Favorability, key = "Type", value = "Favorability") %>%
  mutate(sd = sd(Favorability), se = 1.96 * sd/sqrt(n())) %>%
  ggplot(aes(x = Type, y = Favorability)) +
  geom_col() +
  geom_errorbar(aes(ymin = Favorability- se, ymax = Favorability + se), width=.2, position=position_dodge(.9)) +
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
  labs(title = "Average Disfavorability Score Med vs. Legal 2011", caption = "CAPTION CAPTION CAPTION") +
